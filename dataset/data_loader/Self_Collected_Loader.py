...
class Self_Collected_Loader(BaseLoader):
    """The data loader for the PURE dataset."""

    def __init__(self, name, data_path, config_data):
        """Initializes an Self_Collected dataloader.
            Args:
                data_path(str): path of a folder which stores raw video ,bvp signal and hr data.
    ...
    def preprocess_dataset_subprocess(self, data_dirs, config_preprocess, i, file_list_dict):
        ...
        # Read Labels
        bvps, hrs = self.read_ppg_hr_csv(os.path.join(data_dirs[i]['path'], "{0}_wave.csv".format(filename)))
        if config_preprocess.USE_PSUEDO_PPG_LABEL:
            bvps = self.generate_pos_psuedo_labels(frames, fs=self.config_data.FS)

        target_length = frames.shape[0]
        bvps = BaseLoader.resample_ppg(bvps, target_length)
        hrs = BaseLoader.resample_ppg(hrs, target_length)
        frames_clips, bvps_clips = self.preprocess(frames, bvps, config_preprocess)
        input_name_list, label_name_list = self.save_multi_process(frames_clips, bvps_clips, saved_filename, hrs)
    ...
    @staticmethod
    def read_ppg_hr_csv(bvp_file, wave_fs=60):
